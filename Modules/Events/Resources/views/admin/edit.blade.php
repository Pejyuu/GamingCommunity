...
{{ Form::model($event, array('route' => array('event.update', $event->id), 'method' => 'PUT')) }}{{-- Form model binding to automatically populate our fields with permission data --}}

<h1><i class='fa fa-key'></i> Edit Event</h1>
<hr>
<div class="row">
  ...
  {{   Form::text('location', null, ['class' => 'form-control', 'placeholder' => 'Sted / Adresse']) }}
    ...
    <h5> {{ Form::label('name', 'Featured Image') }}</h5>
      ...
      <div class="input-group">
        ...
        @if($event->filepath)
        <img id="preview" src="{{ url( $event->filepath )}}" style="margin-top:15px; width: 100%;">
        @endif
    ...
    {{ Form::submit('Edit Event', array('class' => 'btn btn-success form-control')) }}
  </div>
